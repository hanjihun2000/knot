...
const UserProfile = () => {
  ...
  const fetchFriendList = () => {
    fetch(
      `http://localhost:8000/api/userapi/viewFollowing?username=${user.username}`
    )
      .then((response) => {
        if (!response.ok) {
  ...
  const sendFollowRequest = async (sender, receiver) => {
    try {
      const response = await fetch(
        "http://localhost:8000/api/followapi/makeFollowRequest",
        {
          method: "POST",
          headers: {
  ...
  useEffect(() => {
    fetch(
      `http://localhost:8000/api/userapi/viewProfilePicture?username=${username}`
    )
      .then((response) => {
        if (!response.ok) {
          throw new Error("Network response was not ok");
  ...
  const renderMedia = (media) => {
    ...
    const blob = new Blob([new Uint8Array(media.buffer.data)], {
      type: media.mimetype,
    });
  ...
  return (
    <div className="user-profile-container">
        ...
        {user.username !== username &&
          (friendList.includes(username) ? (
            ...
            <button
              ...
              onClick={() => sendFollowRequest(user.username, username)}
            >
              Follow
            </button>
          ))}
      ...
      {(friendList.includes(username) && userPrivacy === "private") ||
      userPrivacy !== "private" ||
      user.username === username ||
      userPrivacy === "admin" ? (
              ...
              <div key={post.postId} className="post">
                ...
                <div className="post-actions">
                    ...
                    <>
                      {editingPost && editingPost.postId === post.postId ? (
                        ...
                        <>
                          ...
                          >
                            <PencilSimpleLine className="pencilSimpleLine-icon" />
                          </div>
                          <div
                            alt="Edit"
                            className="action-icon"
                  ...
                  <Link
                    ...
                    className="post-link"
                  >
                    <div alt="Edit" className="action-icon">
                      <MagnifyingGlassPlus className="magnifyingGlassPlus-icon" />
                    </div>
                  </Link>
                </div>
              </div>
        ...
        <div className="private-account-message-container">
          <p className="private-account-message">
            The posts of this user are private. Send a follow request to see
            their posts.
          </p>
        </div>
      )}
